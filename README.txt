===============================================
The Action database field for Moodle >= 2.3
===============================================

   The Action database field acts as an extra API layer to restrict view
   ...
   an Action field, but access for other users can be restricted to "Hidden",
   "Visible (and not editable)" or "Visible and editable".

   Additionally the following special "action" fields are available:
   ...
   (b) unapprove
       ...
       automatically sets records added by teachers/actions as "approved"
...
=================================================
    ...
    1. Clone this plugin to your server
       ...
       cd /PATH/TO/MOODLE
       git clone -q https://github.com/gbateson/moodle-datafield_action.git mod/data/field/action

    2. Add this plugin to the GIT exclude file

       cd /PATH/TO/MOODLE
       echo '/mod/data/field/action/' >> '.git/info/exclude'
    ...
    1. download the zip file from one of the following locations
        ...
        * https://github.com/gbateson/moodle-datafield_action/archive/master.zip
        * http://bateson.kanazawa-gu.ac.jp/moodle/zip/plugins_datafield_action.zip

    2. Unzip the zip file - if necessary renaming the resulting folder to "action".
       Then upload, or move, the "action" folder into the "mod/data/field" folder on
       your Moodle >= 2.3 site, to create a new folder at "mod/data/field/action"
    ...
    3. In Moodle <= 3.1, database plugin strings aren't fully modularised, so the
       ...
       Database activity module, in file "/PATH/TO/MOODLE/mod/data/lang/en/data.php"
          ...
          $string['nameaction'] = 'Action field';

    4. Log in to Moodle as actionistrator to initiate the install/update

       If the install/update does not begin automatically, you can initiate it
       manually by navigating to the following Moodle actionistration page:

          Settings -> Site actionistration -> Notifications
    ...
    (a) the "action" folder is put in the wrong place
        SOLUTION: make sure the folder is at "mod/data/field/action"
                  under your main Moodle folder, and that the file
                  "mod/data/field/action/field.class.php" exists

    (b) permissions are set incorrectly on the "mod/data/field/action" folder
...
=================================================
    ...
    1. Get the latest version of this plugin
       ...
       cd /PATH/TO/MOODLE/mod/data/field/action
       git pull

    2. Log in to Moodle as actionistrator to initiate the update
...
===============================================
To ADD an Action field to a database activity
===============================================

    1. Login to Moodle, and navigate to a course page in which you are a teacher (or action)

    2. Locate, or create, the Database activity to which you wish to add an Action field

    4. click the link to view the Database activity, and then click the "Fields" tab

    5. From the "Field type" menu at the bottom of the page, select "Action"
